...
function StageFrame({ stages, pos, dragging, morphMs, lang, ui }) {
  ...
  return (
    <div className="frame">
          ...
          return (
            ...
            >
              <image-slot
                ...
                placeholder={ui.dropPhoto(s.name)}
      ...
      <div className="frame-tag">
        ...
        <span className="frame-tag-name">{window.ROOM_L(stages[nearest], lang).name}</span>
      </div>
    </div>
  );
}
...
function Scrubber({ stages, pos, onScrub, onSnap, onJump, lang }) {
  const rtl = lang === "ar";
    ...
    <div className="scrubber">
      <div className="scrub-track-wrap">
        <div className="scrub-track" ref={trackRef} onMouseDown={start} onTouchStart={start}>
          ...
          <div className="scrub-handle" style={{ insetInlineStart: `${pct}%` }}>
        ...
        {stages.map((s0, i) => {
          ...
          return (
            <button
              ...
              style={{ insetInlineStart: `${(i / (N - 1)) * 100}%` }}
              onClick={() => onJump(i)}
            >
              {s.name}
            </button>
...
function InfoPanel({ stages, pos, onJump, lang, ui }) {
  const nearest = Math.round(pos);
  const rtl = lang === "ar";
  const s = window.ROOM_L(stages[nearest], lang);
  const N = stages.length;
      ...
      <button className="info-top" onClick={() => setOpen((o) => !o)} aria-expanded={open}>
        ...
        <div className="info-top-right">
          <div className="info-count">
            {ui.stage} <b>{s.n}</b> <span className="of">/ {String(N).padStart(2, "0")}</span>
          </div>
          ...
          <span className="info-toggle-label">{open ? ui.hideDetails : ui.viewDetails}</span>
        </div>
      </button>
      ...
      {open && (
        <React.Fragment>
          <div className="info-body" key={s.key}>
            ...
            <div className="mat-head">{ui.materials}</div>
          ...
          <div className="info-nav">
            <button
              ...
              disabled={nearest === 0}
            >
              <span className="navarrow">{rtl ? "→" : "←"}</span> {ui.prev}
            </button>
            ...
            >
              {ui.next} <span className="navarrow">{rtl ? "←" : "→"}</span>
            </button>
          </div>
        </React.Fragment>
      )}
